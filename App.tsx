...
const App: React.FC = () => {
  ...
  const extractTextFromPdf = async (file: File): Promise<string | null> => {
    try {
        ...
        // Set worker to a reliable CDN source matching the version in package.json/importmap
        ...
        // Use Uint8Array and CMaps for better compatibility with various PDF encodings
        ...
        for (let i = 1; i <= maxPages; i++) {
            ...
            // Join items with a space to separate words, then add newlines between pages
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        if (file.type.startsWith('image/')) {
            ...
            if (!text) {
                addMessage(Sender.SYSTEM, "Could not parse PDF. Please try a text file.");
                return;
            }
            attachment = { type: 'file', mimeType: 'text/plain', data: text, fileName: file.name };
            if (!resumeData && (text.includes("Experience") || text.includes("Education") || text.includes("Skills"))) {
                ...
                addMessage(Sender.SYSTEM, "Resume Context Loaded. I'm reviewing it now...");
                
                // Automatically trigger an intro message from AI based on resume
                setIsLoading(true);
                generateInterviewQuestion("Start the interview based on this resume.", persona, false, { data: text, mimeType: 'text/plain' })
                    .then(res => {
                        setIsLoading(false);
